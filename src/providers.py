...
def build_reflector_command(args: Optional[str] = None) -> Optional[dict[str, object]]:
    ...
    backup_path = settings.get("reflector_backup_path", "").strip()

    if backup_enabled and backup_path and save_path:
        save_q = shlex.quote(save_path)
        backup_q = shlex.quote(backup_path)
        backup_dir = os.path.dirname(backup_path)
        script_lines = []
        script_lines.append(f"if [ -e {save_q} ]; then")
        if backup_dir:
            script_lines.append(f"    mkdir -p {shlex.quote(backup_dir)}")
        script_lines.append(f"    install -Dm644 {save_q} {backup_q}")
        script_lines.append("fi")
        quoted_cmd = " ".join(shlex.quote(part) for part in cmd)
        script_lines.append(quoted_cmd)
        script = "\n".join(script_lines)
        return {"argv": ["/bin/sh", "-c", script], "needs_root": True}
